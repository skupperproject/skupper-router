...
def query():
    ...
    if ip_list:
        ...
        si = ip_list.index(ip)
        ip_list = ip_list[:si]
        return [{"role": "inter-router", "host": "%s" % host} for host in ip_list]
    return ip_list
...
def infer():
    ...
    return [{"role": "inter-router", "host": "%s-%s.%s.%s.svc.cluster.local" % (prefix, i, service_name, namespace)} for
            i in range(int(index))]
