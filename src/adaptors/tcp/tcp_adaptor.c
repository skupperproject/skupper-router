...
static void on_activate(void *context)
{
    ...
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] on_activate", conn->conn_id);
...
static void grant_read_buffers(qdr_tcp_connection_t *conn, const char *msg)
{
    ...
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG,
...
static void qd_free_tcp_adaptor_config(qd_tcp_adaptor_config_t *config)
{
    ...
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_INFO,
...
void qdr_tcp_q2_unblocked_handler(const qd_alloc_safe_ptr_t context)
{
    ...
    if (tc->pn_raw_conn) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] q2 unblocked: call pn_raw_connection_wake()",
               tc->conn_id);
...
static int handle_incoming_raw_read(qdr_tcp_connection_t *conn, qd_buffer_list_t *buffers)
{
    ...
    if (conn->require_tls) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] handle_incoming_raw_read() output=%i",
    ...
    if (result > 0) {
        ...
        if (!window_was_full && read_window_full(conn)) {
            ...
            qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_TRACE,
...
static int handle_incoming(qdr_tcp_connection_t *conn, const char *msg)
{
    ...
    if (conn->q2_blocked) {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, DLV_FMT " handle_incoming q2_blocked for %s connection",
    ...
    if (!conn->in_dlv_stream) {
        ...
        if (conn->require_tls) {
            if (conn->alpn_protocol) {
                ...
                qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] %s No ALPN protocol was negotiated",
    ...
    if (count > 0) {
        ...
        if (conn->q2_blocked) {
            ...
            qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, DLV_FMT " %s client link blocked on Q2 limit",
                   DLV_ARGS(conn->in_dlv_stream), qdr_tcp_connection_role_name(conn));
        }
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_TRACE, DLV_FMT " Continuing %s message with %i bytes",
        ...
        if (stream_data_result == QD_MESSAGE_STREAM_DATA_BODY_OK) {
            ...
            case QD_MESSAGE_STREAM_DATA_ABORTED:
                // treat aborted like end-of-stream since both require closing the connection.
                    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_INFO, "[C%" PRIu64 "] EOS", conn->conn_id);
                    conn->read_eos_seen = true;
                    break;
            case QD_MESSAGE_STREAM_DATA_INVALID:
                    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR,
...
static bool copy_outgoing_buffs(qdr_tcp_connection_t *conn)
{
    ...
    if (conn->outgoing_buff_count == 0) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] Copied %d buffers, %i remain", conn->conn_id,
...
static void handle_outgoing(qdr_tcp_connection_t *conn)
{
    if (conn->out_dlv_stream) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] handle_outgoing() num_buffers_written=%i",
...
static void encrypt_outgoing_tls(qdr_tcp_connection_t *conn, qd_adaptor_buffer_t *unencrypted_buff, bool write_buffers)
{
    ...
    if (write_buffers && DEQ_SIZE(conn->out_buffs) > 0) {
        assert(!IS_ATOMIC_FLAG_SET(&conn->raw_closed_write));
        int num_buffers_written = qd_raw_connection_write_buffers(conn->pn_raw_conn, &conn->out_buffs);
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] encrypt_outgoing_tls() num_buffers_written=%i",
               conn->conn_id, num_buffers_written);
    }
}
...
static void handle_connection_event(pn_event_t *e, qd_server_t *qd_server, void *context)
{
    ...
    case PN_RAW_CONNECTION_CLOSED_READ: {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] PN_RAW_CONNECTION_CLOSED_READ %s",
               conn->conn_id, qdr_tcp_connection_role_name(conn));
        break;
    }
    ...
    case PN_RAW_CONNECTION_NEED_WRITE_BUFFERS: {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] PN_RAW_CONNECTION_NEED_WRITE_BUFFERS %s",
               conn->conn_id, qdr_tcp_connection_role_name(conn));
        while (qdr_connection_process(conn->qdr_conn)) {}
        handle_outgoing(conn);
        break;
    ...
    case PN_RAW_CONNECTION_NEED_READ_BUFFERS: {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] PN_RAW_CONNECTION_NEED_READ_BUFFERS %s",
               conn->conn_id, qdr_tcp_connection_role_name(conn));
        while (qdr_connection_process(conn->qdr_conn)) {}
        if (conn->in_dlv_stream) {
    ...
    case PN_RAW_CONNECTION_WAKE: {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] PN_RAW_CONNECTION_WAKE %s", conn->conn_id,
               qdr_tcp_connection_role_name(conn));
        if (sys_atomic_set(&conn->q2_restart, 0)) {
            ...
            UNLOCK(&conn->activation_lock);
            // note: unit tests grep for this log!
            qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_TRACE, "[C%" PRIu64 "] %s client link unblocked from Q2 limit",
                   conn->conn_id, qdr_tcp_connection_role_name(conn));
    ...
    default:
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] Unexpected Event: %d", conn->conn_id,
               pn_event_type(e));
        break;
    }
}
...
static void qdr_tcp_create_server_side_connection(qdr_tcp_connection_t* tc)
{
    ...
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_INFO, "[C%" PRIu64 "] Opening server-side core connection %s", tc->conn_id,
           host);
    ...
    tc->outgoing_link = qdr_link_first_attach(conn,
                                              ...
                                              &(tc->outgoing_link_id));
    if (!!tc->initial_delivery) {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, DLV_FMT " initial_delivery ownership passed to " DLV_FMT,
...
static bool qdr_tcp_create_egress_connection(qd_tcp_connector_t *connector, qdr_delivery_t *initial_delivery)
{
    ...
    if (tc->require_tls) {
        ...
        if (tc->alpn_protocol) {
            ...
            if (res != 0) {
            qd_log(QD_LOG_MODULE_TCP_ADAPTOR,
                   QD_LOG_ERROR,
...
{
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_INFO, "Configured %s for %s, %s:%s", what, c->adaptor_config->address,
...
QD_EXPORT qd_tcp_listener_t *qd_dispatch_configure_tcp_listener(qd_dispatch_t *qd, qd_entity_t *entity)
{
    ...
    if (qd_load_tcp_adaptor_config(li->config, entity) != QD_ERROR_NONE) {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "Unable to create tcp listener: %s", qd_error_message());
    ...
    if (li->config->adaptor_config->ssl_profile_name) {
        ...
        li->tls_domain = qd_tls_domain(li->config->adaptor_config, qd, QD_LOG_MODULE_TCP_ADAPTOR, tcp_alpn_protocols,
    ...
    li->adaptor_listener = qd_adaptor_listener(qd, li->config->adaptor_config, QD_LOG_MODULE_TCP_ADAPTOR);
    qd_adaptor_listener_listen(li->adaptor_listener, qdr_tcp_connection_ingress, (void*) li);

    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "tcpListener %s created at %s:%s for address %s",
...
QD_EXPORT void qd_dispatch_delete_tcp_listener(qd_dispatch_t *qd, void *impl)
{
    ...
    if (li) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_INFO, "Deleted TcpListener for %s, %s:%s",
...
QD_EXPORT qd_tcp_connector_t *qd_dispatch_configure_tcp_connector(qd_dispatch_t *qd, qd_entity_t *entity)
{
    ...
    if (qd_load_tcp_adaptor_config(c->config, entity) != QD_ERROR_NONE) {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "Unable to create tcp connector: %s", qd_error_message());
        qd_tcp_connector_decref(c);
        return 0;
    }

    if (c->config->adaptor_config->ssl_profile_name) {
        c->tls_domain = qd_tls_domain(c->config->adaptor_config, qd, QD_LOG_MODULE_TCP_ADAPTOR, 0, 0, false);
        if (!c->tls_domain) {
            // note qd_tls_domain() logged the error
            qd_tcp_connector_decref(c);
            return 0;
...
QD_EXPORT void qd_dispatch_delete_tcp_connector(qd_dispatch_t *qd, void *impl)
{
    ...
    if (ct) {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_INFO, "Deleted TcpConnector for %s, %s:%s",
                                 ...
                                 qd_session_class_t session_class)
{
    ...
    if (tcontext) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] qdr_tcp_first_attach: NOOP",
               conn->conn_id, qdr_tcp_conn_linkid(conn));
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_first_attach: no link context");
        assert(false);
    }
}
                                  ...
                                  qdr_terminus_t *source, qdr_terminus_t *target)
{
    ...
    if (link_context) {
        ...
        if (qdr_link_direction(link) == QD_OUTGOING) {
            qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] %s qdr_tcp_second_attach",
            ...
            if (tc->ingress) {
                ...
                qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] qdr_tcp_flow qd_tls_has_output=true",
                       tc->conn_id);
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_second_attach: no link context");
        assert(false);
    }
}
...
{
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_detach");
    assert(false);
}
...
static void qdr_tcp_flow(void *context, qdr_link_t *link, int credit)
{
    ...
    if (link_context) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_flow: no link context");
        assert(false);
    }
}
...
static void qdr_tcp_offer(void *context, qdr_link_t *link, int delivery_count)
{
    ...
    if (link_context) {
        qdr_tcp_connection_t* conn = (qdr_tcp_connection_t*) link_context;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] qdr_tcp_offer: NOOP",
               conn->conn_id, qdr_tcp_conn_linkid(conn));
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_offer: no link context");
        assert(false);
    }
...
static void qdr_tcp_drained(void *context, qdr_link_t *link)
{
    ...
    if (link_context) {
        qdr_tcp_connection_t* conn = (qdr_tcp_connection_t*) link_context;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] qdr_tcp_drained: NOOP",
               conn->conn_id, qdr_tcp_conn_linkid(conn));
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_drained: no link context");
        assert(false);
    }
}
...
static void qdr_tcp_drain(void *context, qdr_link_t *link, bool mode)
{
    ...
    if (link_context) {
        qdr_tcp_connection_t* conn = (qdr_tcp_connection_t*) link_context;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] qdr_tcp_drain: NOOP",
               conn->conn_id, qdr_tcp_conn_linkid(conn));
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_drain: no link context");
        assert(false);
    }
}
...
static int qdr_tcp_push(void *context, qdr_link_t *link, int limit)
{
    ...
    if (link_context) {
        qdr_tcp_connection_t* conn = (qdr_tcp_connection_t*) link_context;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] qdr_tcp_push", conn->conn_id,
               qdr_tcp_conn_linkid(conn));
        return qdr_link_process_deliveries(tcp_adaptor->core, link, limit);
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_push: no link context");
        assert(false);
        return 0;
    }
}
...
static uint64_t qdr_tcp_deliver(void *context, qdr_link_t *link, qdr_delivery_t *delivery, bool settled)
{
    ...
    if (link_context) {
        qdr_tcp_connection_t* tc = (qdr_tcp_connection_t*) link_context;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, DLV_FMT " qdr_tcp_deliver Delivery event", DLV_ARGS(delivery));
        if (tc->is_egress_dispatcher_conn) {
            ...
            if (tc->ingress) {
                ...
                tc->incoming_link = qdr_link_first_attach(tc->qdr_conn,
                                                          ...
                                                          NULL,
                                                          &(tc->incoming_link_id));
                qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] %s Created link to %s",
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_deliver: no link context");
        assert(false);
    }
    return 0;
}
...
static int qdr_tcp_get_credit(void *context, qdr_link_t *link)
{
    ...
    if (link_context) {
        qdr_tcp_connection_t* conn = (qdr_tcp_connection_t*) link_context;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] qdr_tcp_get_credit: NOOP",
               conn->conn_id, qdr_tcp_conn_linkid(conn));
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_get_credit: no link context");
        assert(false);
    }
    return 10;
}
...
static void qdr_tcp_delivery_update(void *context, qdr_delivery_t *dlv, uint64_t disp, bool settled)
{
    ...
    if (link_context) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_delivery_update: no link context");
        assert(false);
    }
}
...
static void qdr_tcp_conn_close(void *context, qdr_connection_t *conn, qdr_error_t *error)
{
    ...
    if (tcontext) {
        ...
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_conn_close: no connection context");
        assert(false);
    }
}
...
static void qdr_tcp_conn_trace(void *context, qdr_connection_t *conn, bool trace)
{
    ...
    if (tcontext) {
        qdr_tcp_connection_t* conn = (qdr_tcp_connection_t*) tcontext;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] qdr_tcp_conn_trace: NOOP",
               conn->conn_id, qdr_tcp_conn_linkid(conn));
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_ERROR, "qdr_tcp_conn_trace: no connection context");
        assert(false);
    }
}
...
static void qdr_tcp_activate_CT(void *notused, qdr_connection_t *c)
{
    ...
    if (context) {
        ...
        if (conn->pn_raw_conn && !(IS_ATOMIC_FLAG_SET(&conn->raw_closed_read) && IS_ATOMIC_FLAG_SET(&conn->raw_closed_write))) {
            ...
            UNLOCK(&conn->activation_lock);
            qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] qdr_tcp_activate_CT: Cannot activate",
                   conn->conn_id);
        }
    } else {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "qdr_tcp_activate_CT: no connection context");
...
{
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_INFO, "Shutting down TCP protocol adaptor");
...
{
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "Insert column %i for %p", col, (void *) conn);
    char id_str[100];
...
{
    qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "query for first tcp connection (%i)", offset);
                               ...
                               const char          *qdr_tcp_connection_columns[])
{
    ...
    if (!identity) {
        ...
        qd_log(QD_LOG_MODULE_AGENT, QD_LOG_ERROR, "Error performing READ of %s: %s", TCP_CONNECTION_TYPE,
...
static void qdr_add_tcp_connection_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    if (!discard) {
        ...
        conn->in_list = true;
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "] qdr_add_tcp_connection_CT %s (%zu)",
...
static void detach_links(qdr_tcp_connection_t *conn)
{
    if (conn->incoming_link) {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] detaching incoming link",
               conn->conn_id, conn->incoming_link_id);
        qdr_link_detach(conn->incoming_link, QD_LOST, 0);
        conn->incoming_link = 0;
    }
    if (conn->outgoing_link) {
        qd_log(QD_LOG_MODULE_TCP_ADAPTOR, QD_LOG_DEBUG, "[C%" PRIu64 "][L%" PRIu64 "] detaching outgoing link",
