 ...
 *
 */
static bool check_alpn_protocol_match(qdr_http2_connection_t *http_conn)
{
    const char *protocol_name;
    size_t protocol_name_length;
    bool        alpn_check_success = true;
    ...
    if (conn->require_tls) {
        ...
        if (encrypted_bytes_in == QD_TLS_ERROR) {
            ...
            if (!conn->alpn_check_complete)
                close_conn = !check_alpn_protocol_match(conn);
