                                     ...
                                     qd_parsed_field_t *in_body)
{
    while (true) {
        ...
        //
        al = qdr_route_add_auto_link_CT(core, name, addr_field, dir, container_field, connection_field, external_addr);
    ...
    if (query->body) {
        ...
        if (query->status.status / 100 > 2)
            qd_log(
                QD_LOG_MODULE_ROUTER_CORE, QD_LOG_ERROR, "Error configuring autoLink: %s", query->status.description);
