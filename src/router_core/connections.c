...
static void qdr_connection_group_member_cleanup_CT(qdr_core_t *core, qdr_connection_t *conn)
{
    ...
    if (!!correlator[0]) {
        ...
        if (qd_bitmask_valid_bit_value(conn->group_parent_mask_bit)) {
            ...
            assert(!!parent
                   && strcmp(parent->connection_info->group_correlator, correlator) == 0
                   && parent->connection_info->group_ordinal == ordinal);
...
static void qdr_detach_link_control_CT(qdr_core_t *core, qdr_connection_t *conn, qdr_link_t *link)
{
    ...
    if (link->link_direction == QD_OUTGOING) {
        ...
        // heartbeat timeout). This should only be done if the connection is the active control connection:
        //
        if (qd_bitmask_valid_bit_value(conn->mask_bit) && !!core->rnode_conns_by_mask_bit[conn->mask_bit]) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG,
...
static void setup_inter_router_control_conn_CT(qdr_core_t *core, qdr_connection_t *conn)
{
    ...
    //
    // Connection group upgrade: see if there is already an inter-router connection using the same group correlator. If
    // so we're dealing with upgrading an existing inter-router connection to a new one.
    //
    if (!!correlator[0]) {
        for (int idx = 0; idx < qd_bitmask_width(); ++idx) {
            qdr_connection_t *tmp = core->rnode_conns_by_mask_bit[idx];
    ...
    if (!old_conn) {
        ...
        // There is a pre-existing control connection for the given group. We must upgrade to the connection with the
        // greater group ordinal value. Check if the new connection can upgrade the old.
        //
        qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "Upgrading inter-router connections [C%"PRIu64"] and [C%"PRIu64"]",
               old_conn->identity, conn->identity);

        if (old_conn->connection_info->group_ordinal >= conn->connection_info->group_ordinal) {
            // New connection has lesser ordinal so avoid upgrading to it.
            //
            qd_log(LOG_ROUTER_CORE, QD_LOG_WARNING, "Existing connection [C%"PRIu64"] takes precedence, closing [C%"PRIu64"]",
                   old_conn->identity, conn->identity);
            conn->error = qdr_error(QD_AMQP_COND_CONNECTION_FORCED, "Existing connection takes precedence");
            qdr_close_connection_CT(core, conn);

        } else if (core->pending_rnode_conns_by_mask_bit[old_conn->mask_bit] != 0) {
            // There is already a connection upgrade in progress. Close the new connection with the expectation that the
            // upgrade will complete before the new connection reconnects. On reconnect this code will be executed
            // again.
            //
            qd_log(LOG_ROUTER_CORE, QD_LOG_WARNING, "Upgraded for [C%"PRIu64"] in progress, closing [C%"PRIu64"]",
                   old_conn->identity, conn->identity);
            conn->error = qdr_error(QD_AMQP_COND_CONNECTION_FORCED, "Upgrade in progress - retry later");
            qdr_close_connection_CT(core, conn);

        } else {
            qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "Upgrading connection [C%"PRIu64"] to [C%"PRIu64"]",
                   old_conn->identity, conn->identity);
