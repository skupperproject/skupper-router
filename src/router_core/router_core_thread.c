...
void qdr_modules_init(qdr_core_t *core)
{
    ...
    while (module) {
        ...
        if (module->enabled) {
            ...
            qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_INFO, "Core module enabled: %s", module->name);
        } else
            qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_INFO, "Core module present but disabled: %s", module->name);
...
void qdr_modules_finalize(qdr_core_t *core)
{
    ...
    while (module) {
        if (module->enabled) {
            qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_INFO, "Finalizing core module: %s", module->name);
...
void *router_core_thread(void *arg)
{
    ...
    qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_INFO, "Router Core thread running. %s/%s", core->router_area,
           core->router_id);
    while (core->running) {
        //
        ...
        if (bg_action) {
            if (bg_action->label)
                qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_TRACE, "Core background action '%s'%s", bg_action->label,
        ...
        while (action) {
            ...
            if (action->label)
                qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_TRACE, "Core action '%s'%s", action->label,
                       core->running ? "" : " (discard)");
    ...
    qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_INFO, "Router Core thread exited");
    return 0;
}
