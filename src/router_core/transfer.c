                                 ...
                                 qd_delivery_state_t *remote_state)
{
    ...
    qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_DEBUG, DLV_FMT " Delivery created qdr_link_deliver", DLV_ARGS(dlv));
                                    ...
                                    qd_delivery_state_t *remote_state)
{
    ...
    dlv->conn_id            = link->conn_id;
    sys_mutex_init(&dlv->dispo_lock);
    qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_DEBUG, DLV_FMT " Delivery created qdr_link_deliver_to", DLV_ARGS(dlv));
                                                ...
                                                qd_delivery_state_t* remote_state)
{
    ...
    qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_DEBUG, DLV_FMT " Delivery created qdr_link_deliver_to_routed_link",
           DLV_ARGS(dlv));
                ...
                if (send_complete || new_disp == QD_DELIVERY_MOVED_TO_NEW_LINK) {
                    ...
                    if (new_disp != QD_DELIVERY_MOVED_TO_NEW_LINK) {
                        ...
                        if (settled || qdr_delivery_oversize(dlv) || qdr_delivery_is_aborted(dlv)) {
                            ...
                            qd_log(
                                QD_LOG_MODULE_ROUTER_CORE, QD_LOG_DEBUG,
                                DLV_FMT
...
static void qdr_link_deliver_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    if (DEQ_IS_EMPTY(link->undelivered)) {
        ...
        if (addr && addr->router_control_only && link->link_type != QD_LINK_CONTROL) {
            qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_DEBUG, DLV_FMT " Link forward:  qdr_link_deliver_CT: released dlv",
                   DLV_ARGS(dlv));
            qdr_delivery_release_CT(core, dlv);
            qdr_link_issue_credit_CT(core, link, 1, false);
...
void qdr_in_process_send_to_CT(qdr_core_t *core, qd_iterator_t *address, qd_message_t *msg, bool exclude_inprocess, bool control)
{
    ...
    if (addr) {
        ...
        qd_log(QD_LOG_MODULE_ROUTER_CORE, QD_LOG_DEBUG, "In-process send to an unknown address");
}
