...
static uint64_t _client_on_reply_cb(qdr_core_t    *core,
                                    ...
                                    qd_iterator_t *body)
{
    qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test reply received rc=%p", request_context);
                              ...
                              uint64_t       disposition)
{
    ...
    qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test request ack rc=%p d=%" PRIu64, request_context,
           disposition);
                               ...
                               const char    *error) TA_REQ(core_thread_capability)
{
    ...
    test_client_t *tc = (test_client_t *)user_context;
    qd_log_level_t level = (error) ? QD_LOG_ERROR : QD_LOG_DEBUG;
...
{
    qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test on state active=%c", active ? 'T' : 'F');
}
                               ...
                               bool drain) TA_REQ(core_thread_capability)
{
    ...
    qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test on flow c=%d d=%c", available_credit,
           drain ? 'T' : 'F');
...
static void _on_conn_event(void *context, qdrc_event_t type, qdr_connection_t *conn) TA_REQ(core_thread_capability)
{
    ...
    qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test on conn event");

    switch (type) {
    case QDRC_EVENT_CONN_OPENED:
        qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test conn open");
        ...
        if (cid && strcmp(cid, "io.skupper.router.test_core_client") == 0) {
            qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test connection opened");
    ...
    case QDRC_EVENT_CONN_CLOSED:
        qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test conn closed");
        if (tc->conn == conn) {
            tc->conn = NULL;
            tc->credit = 0;
            tc->counter = 0;
            ...
            tc->core_client = NULL;
            qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test connection closed");
        }
        break;
    }
}
...
static void qdrc_test_client_api_setup(test_module_t *test_module)
{
    ...
    qd_log(LOG_ROUTER_CORE, QD_LOG_DEBUG, "client test registered %p", (void *) tc->conn_events);
}
