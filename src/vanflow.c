...
#define LINE_MAX 1000
    ...
    qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "%s", line);
}
...
static void _vflow_all_address_status_TH(vflow_work_t *work, bool discard)
{
    ...
    if (now_usable && !state->all_address_usable) {
        ...
        //
        qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "Event collector detected.  Begin sending beacons.");
        ...
        //
        qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "Event collector lost.  Stop sending beacons.");
...
static void _vflow_my_address_status_TH(vflow_work_t *work, bool discard)
{
    ...
    if (now_usable && !state->my_address_usable) {
        ...
        //
        qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO,
        ...
        //
        qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "Event collector for this router lost.  Stop sending flow events.");
...
static void _vflow_my_flow_address_status_TH(vflow_work_t *work, bool discard)
{
    ...
    if (now_usable && !state->my_flow_address_usable) {
        ...
        // Start sending log records
        //
        qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "Event collection for flow events detected.  Begin sending flow events.");
        state->my_flow_address_usable = true;
    } else if (!now_usable && state->my_flow_address_usable) {
        //
        // Stop sending log records
        //
        qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "Event collection for flow events ended.  Stop sending flow events.");
...
static void *_vflow_thread_TH(void *context)
{
    ...
    qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "Protocol logging started");
    ...
    qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "Protocol logging completed");
    return 0;
}
...
void vflow_set_ref_from_parsed(vflow_record_t *record, vflow_attribute_t attribute_type, qd_parsed_field_t *field)
{
    if (!!record) {
        ...
        if (!!work->value.string_val) {
            ...
            free_vflow_work_t(work);
            qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_WARNING, "Reference ID cannot be parsed from the received field");
        }
    }
}
    ...
    if (qd_message_check_depth(msg, QD_DEPTH_PROPERTIES) == QD_MESSAGE_DEPTH_OK) {
        ...
        if (!!subject_iter) {
            if (qd_iterator_equal(subject_iter, (const unsigned char*) "FLUSH")) {
                qd_log(QD_LOG_MODULE_FLOW_LOG, QD_LOG_INFO, "FLUSH request received");
