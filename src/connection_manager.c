 ...
 */
static void qd_set_password_from_file(const char *password_file, char **password_field)
{
    if (password_file) {
        ...
        if (file == NULL) {
            ...
            //
            qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_ERROR, "Unable to open password file %s, error: %s", password_file,
                   strerror(errno));
            return;
        }
...
static void qd_config_process_password(char **actual_val, char *pw, bool *is_file, bool allow_literal_prefix)
{
    if (!pw)
        return;
    ...
    //
        ...
        qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_WARNING,
...
QD_EXPORT qd_config_ssl_profile_t *qd_dispatch_configure_ssl_profile(qd_dispatch_t *qd, qd_entity_t *entity)
{
    ...
    if (ssl_profile->ssl_password) {
        ...
        char *actual_pass = 0;
        bool is_file_path = 0;
        qd_config_process_password(&actual_pass, ssl_profile->ssl_password, &is_file_path, true);
        CHECK();
        if (actual_pass) {
            if (is_file_path) {
    ...
    qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_INFO, "Created SSL Profile with name %s ", ssl_profile->name);
    return ssl_profile;

    error:
    qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_ERROR, "Unable to create ssl profile: %s", qd_error_message());
...
{
    qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_INFO, "Configured %s: %s proto=%s, role=%s%s%s%s", what, c->host_port,
...
QD_EXPORT qd_listener_t *qd_dispatch_configure_listener(qd_dispatch_t *qd, qd_entity_t *entity)
{
    ...
    if (!li || load_server_config(qd, &li->config, entity, true, 0) != QD_ERROR_NONE) {
        qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_ERROR, "Unable to create listener: %s", qd_error_message());
    ...
    if (fol) {
        ...
        if (li->config.failover_list == 0) {
            qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_ERROR, "Unable to create listener, bad failover list: %s",
                   qd_error_message());
            qd_listener_decref(li);
            return 0;
        }
...
QD_EXPORT qd_connector_t *qd_dispatch_configure_connector(qd_dispatch_t *qd, qd_entity_t *entity)
{
    ...
    if (ct && load_server_config(qd, &ct->config, entity, false, 0) == QD_ERROR_NONE) {
        ...
        //
          if (!strcmp("auto", ct->config.data_connection_count)) {
              ...
              qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_INFO, "Inter-router data connections calculated at %d ",
                     connection_count);
              ...
              qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_INFO, "Inter-router data connections set to %d ", connection_count);
          } else {
              // The user has entered a non-numeric value that is not 'auto'.
              // This is not a legal value. Default to 'auto' and mention it.
              qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_INFO, "Bad value \"%s\" for dataConnectionCount ",
  ...
  error:
    qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_ERROR, "Unable to create connector: %s", qd_error_message());
    ...
    while (li) {
        if (!li->pn_listener) {
            if (!qd_listener_listen(li) && first_start) {
                qd_log(QD_LOG_MODULE_CONN_MGR, QD_LOG_CRITICAL, "Listen on %s failed during initial config",
                       li->config.host_port);
                exit(1);
            } else {
