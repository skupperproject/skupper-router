...
qd_error_t qd_error_vimpl(qd_error_t code, const char *file, int line, const char *fmt, va_list ap) {
    ...
    if (code) {
        ...
        qd_log_impl(LOG_ERROR, QD_LOG_ERROR, file, line, "%s", qd_error_message());
        return code;
    }
    else
        qd_error_clear();
...
{
    if (!qd_log_enabled(LOG_ERROR, level))
        return;
	...
	if (trace) {
            if (strlen(trace) < QD_LOG_TEXT_MAX) {
                qd_log_impl(LOG_ERROR, level, file, line, "%s", trace);
            } else {
                ...
                qd_log_impl(LOG_ERROR, level, file, line, "Traceback truncated:\n%s", tail ? tail : "");
            }
            free(trace);
        }
        Py_DECREF(result);
...
qd_error_t qd_error_errno_impl(int errnum, const char *file, int line, const char *fmt, ...) {
    if (errnum) {
        ...
        qd_log_impl(LOG_ERROR, QD_LOG_ERROR, file, line, "%s", em);
        return qd_error_code();
    }
    else
