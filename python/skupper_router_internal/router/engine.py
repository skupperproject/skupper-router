...
##
from ..dispatch import IoAdapter, LogAdapter, LOG_INFO, LOG_DEBUG, LOG_ERROR, LOG_WARNING, LOG_STACK_LIMIT
...
class RouterEngine:
    ...
    def __init__(self, router_adapter, router_id, area, max_routers, config_override=None):
        ...
        self.io_adapter     = [IoAdapter(self.receive, "qdrouter", 'L', TREATMENT_MULTICAST_FLOOD),
                               IoAdapter(self.receive, "qdrouter", 'T', TREATMENT_MULTICAST_FLOOD),
                               IoAdapter(self.receive, "qdhello",  'L', TREATMENT_MULTICAST_FLOOD)]
    ...
    def handleControlMessage(self, opcode, body, link_id, cost):
        ...
        try:
            ...
            if opcode == 'HELLO':
                ...
                self.log_hello(LOG_DEBUG, "RCVD: %r" % msg)
                self.hello_protocol.handle_hello(msg, now, link_id, cost)

            elif opcode == 'RA':
                msg = MessageRA(body)
                self.log_ls(LOG_DEBUG, "RCVD: %r" % msg)
