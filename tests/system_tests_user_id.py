...
class QdSSLUseridTest(TestCase):
    ...
    def test_ssl_user_id(self):
        ...
        user_id = node.query(type='io.skupper.router.connection', attribute_names=['user']).results[0][0]
        ...
        self.assertEqual("72d543690cb0a8fc2d0f4c704c65411b9ee8ad53839fced4c720d73e58e4f0d7",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[1][0])
        ...
        self.assertEqual("c6de3a340014b0f8a1d2b41d22e414fc5756494ffa3c8760bbff56f3aa9f179a5a6eae09413fd7a6afbf36b5fb4bad8795c2836774acfe00a701797cc2a3a9ab",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[2][0])
        ...
        self.assertEqual("72d543690cb0a8fc2d0f4c704c65411b9ee8ad53839fced4c720d73e58e4f0d7;127.0.0.1;Client;Dev;US;NC",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[3][0])

        addr = self.address(4).replace("amqp", "amqps")
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        self.assertEqual("3eccbf1a2f3e46da823c63a9da9158983cb495a3;US;NC",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[4][0])
        ...
        self.assertEqual("US;NC;c6de3a340014b0f8a1d2b41d22e414fc5756494ffa3c8760bbff56f3aa9f179a5a6eae09413fd7a6afbf36b5fb4bad8795c2836774acfe00a701797cc2a3a9ab",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[5][0])

        addr = self.address(6).replace("amqp", "amqps")
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        self.assertEqual("127.0.0.1;NC;Dev;US;Client",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[6][0])

        addr = self.address(7).replace("amqp", "amqps")
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        self.assertEqual("NC;US;Client;Dev;127.0.0.1;Raleigh",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[7][0])

        addr = self.address(8).replace("amqp", "amqps")
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        self.assertEqual("C=US,ST=NC,L=Raleigh,OU=Dev,O=Client,CN=127.0.0.1",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[8][0])

        addr = self.address(9).replace("amqp", "amqps")
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        self.assertEqual("C=US,ST=NC,L=Raleigh,OU=Dev,O=Client,CN=127.0.0.1",
                         node.query(type='io.skupper.router.connection', attribute_names=['user']).results[9][0])

        addr = self.address(10).replace("amqp", "amqps")
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        user = node.query(type='io.skupper.router.connection', attribute_names=['user']).results[10][0]
        ...
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        user = node.query(type='io.skupper.router.connection', attribute_names=['user']).results[12][0]
        self.assertEqual("user12", str(user))

        addr = self.address(13).replace("amqp", "amqps")
        node = Node.connect(addr, timeout=TIMEOUT, ssl_domain=domain)
        user_id = node.query(type='io.skupper.router.connection', attribute_names=['user']).results[13][0]
