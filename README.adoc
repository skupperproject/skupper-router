...
NOTE: You cannot build Skupper router on Windows.

=== Dependencies

To build dispatch on a yum-based Linux system, you need the following packages installed:
...
Since run time validation slows down `skrouter` considerably it is disabled by default.  
