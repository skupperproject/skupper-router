...
It provides flexible and scalable interconnect between any AMQP endpoints, and is focussed on the (Skupper)[https://skupper.io] use case.

== Building and testing

NOTE: Skupper router will not build on Windows.
...
Since run time validation slows down skrouter considerably it is
...
The Skupper Router test suite supports the following run time
validation tools:

==== Valgrind Memcheck
Runs skrouterd under Valgrind's memcheck leak checker during the CTest
...
If valgrind detects errors the skrouterd process will exit with an
...
skrouterd process will exit with an exit code of 66. This will be
